...
import { Text, ScrollView, View, TextInput, Button, FlatList, TouchableOpacity } from 'react-native';
...
class Home extends React.Component {
    ...
    componentDidMount = async () => {
        try {
            this.setState({ auth: { ...this.state.auth, 'id': await AsyncStorage.getItem('id'), 'token': await AsyncStorage.getItem('token') } })
    ...
    getParams = () => {
        ...
        console.log('comparison: ' + (Object.entries(this.state).toString() === Object.entries(initialState).toString()));
        var params = '';
        if (Object.entries(this.state).toString() === Object.entries(initialState).toString()) {
            return "";
        } else {
            console.log('got here');
            if (!this.state.initialState.name.toString() == initialState.name.toString()) {
                params = params + `q=${this.state.initialState.name}&`
            }
            if (!this.state.initialState.overall_rating == initialState.overall_rating) {
                params = params + `overall_rating=${this.state.initialState.overall_rating}&`
            }
            if (!this.state.initialState.price_rating == initialState.price_rating) {
                params = params + `price_rating=${this.state.initialState.price_rating}&`
            }
            if (!this.state.initialState.quality_rating == initialState.quality_rating) {
                params = params + `quality_rating=${this.state.initialState.quality_rating}&`
            }
            if (!this.state.initialState.clenliness_rating == initialState.clenliness_rating) {
                params = params + `clenliness_rating=${this.state.initialState.clenliness_rating}&`
            }
            if (!this.state.initialState.search_in.toString() == initialState.search_in.toString()) {
    ...
    searchCoffee = async () => {
        ...
        try {
            ...
            let response = await fetch('http://10.0.2.2:3333/api/1.0.0/find?' + params,
                {
                    method: 'GET',
                    headers: {
                ...
                renderItem={({ item }) => (
                    <>
                        <TouchableOpacity onPress={() => {
                            this.props.navigation.navigate("SearchResults", { location: item })
                        }}>
                            <View>
        ...
        return (
            <View>
                ...
                {/* <View >
                    ...
                    <Stars
                        half={true}
                        default={this.state.initialState.price_rating}
                        update={(val) => this.handleParams("price_rating", val)}
                    />
                </View>
                <View >
                    ...
                    <Stars
                        half={true}
                        default={this.state.initialState.quality_rating}
                        update={(val) => this.handleParams('quality_rating', val)}
                ...
                <View>
                    ...
                    <Stars
                        half={true}
                        default={this.state.initialState.cleanliness_rating}
                        update={(val) => this.handleParams('cleanliness_rating', val)}
                    />
                </View> */}
                <View>
                    <Text>Search In</Text>
